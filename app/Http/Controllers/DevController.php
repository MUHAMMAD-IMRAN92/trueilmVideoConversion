    ...
    public function post(Request $request)
    {
        ...
        if (in_array($video_extension, $allowedextentions)) {
            ...
            foreach ($renditions as $index => $rendition) {
                ...
                $outputPathS3 = $outputDirectory . '/rendition_' . $index . '.m3u8';
                Storage::disk('s3')->put($outputPathS3, file_get_contents($outputPath));

                // Upload TS segments
                $tsSegments = glob($outputPath . '_segment_*.ts');
                foreach ($tsSegments as $tsSegment) {
                    $tsSegmentS3 = $outputDirectory . '/' . basename($tsSegment);
                    Storage::disk('s3')->put($tsSegmentS3, file_get_contents($tsSegment));
                }
            }
            ...
            $masterPlaylistPath = $outputDirectory . '/' . $nameWithoutExtension . '.m3u8';
            file_put_contents($masterPlaylistPath, $masterPlaylist);

            // Upload the HLS master playlist to S3
            $masterPlaylistPathS3 = $outputDirectory . '/' . $nameWithoutExtension . '.m3u8';
            Storage::disk('s3')->put($masterPlaylistPathS3, file_get_contents($masterPlaylistPath));
        } else {
            $data['file'] = '';
            $data['poster'] = '';
        }
