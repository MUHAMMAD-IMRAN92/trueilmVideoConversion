...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#ajax-table-category').on('change', function() {
            ...
            ebooktable.ajax.url('<?= url('all-book') ?>?category=' + category + '&price=' + price +
                ...
                .load();
            bookTableFiltersOnchange(price, category, aproval, author, uncategorized);
        });
        ...
        function bookTableFiltersOnchange(price, category, aproval, author, uncategorized) {
            ...
            if (type) {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ url('ajax/top-read-book-data/') }}/" + type,
                    dataType: "json",
                    data: {
                        price: price,
                        category: category,
                    ...
                    success: function(response) {
                        var existing_chart = Chart.getChart('topReadBookChart');
                        existing_chart.destroy();
                        var ctx = document.getElementById('topReadBookChart').getContext('2d');
            ...
            "columns": [{
                ...
                },
                {
                    "mRender": function(data, type, row) {
                        ...
                        if(approve_per){
                            var b=`<a  class="ml-1" href="{{ url('book/approve/`+row._id+`') }}"><i class="fa fa-check" ></i></a>`;

                        }
                        return `<td class="d-flex">` +b+
                            a +
                            `
                                ` +
                            anchor +
    ...
    $('#e_date_dashboard').on('change', function() {
        console.log('ok');
        var e_date = $(this).val();
        var s_date = $('#s_date_dashboard').val();
